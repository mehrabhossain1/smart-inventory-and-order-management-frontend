            ...
            <Table>
                <TableHeader>
                    <TableRow className="bg-slate-50/50 dark:bg-slate-800/30 hover:bg-slate-50/50 dark:hover:bg-slate-800/30">
                        ...
                        <TableHead className="text-xs uppercase tracking-wider text-slate-400 dark:text-slate-500 font-semibold">Category</TableHead>
                        <TableHead className="text-xs uppercase tracking-wider text-slate-400 dark:text-slate-500 font-semibold">Price</TableHead>
                        <TableHead className="text-xs uppercase tracking-wider text-slate-400 dark:text-slate-500 font-semibold">Qty</TableHead>
                ...
                <TableBody>
                    {products.map((product) => {
                        ...
                        return (
                            <TableRow key={product._id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors">
                                ...
                                <TableCell className="text-slate-400 dark:text-slate-500 text-sm">{categoryName}</TableCell>
                                <TableCell className="text-slate-600 dark:text-slate-400 tabular-nums">{formatCurrency(product.price)}</TableCell>
                                <TableCell className="tabular-nums font-medium text-slate-600 dark:text-slate-400">{product.quantity}</TableCell>
                                <TableCell className="tabular-nums text-slate-400 dark:text-slate-500">{product.minimumStockThreshold}</TableCell>
                                <TableCell>
                                    <Badge variant="outline" className={`text-[11px] font-medium px-2.5 ${statusColors[product.status] || ""}`}>
                                        {product.status}
                                    </Badge>
                                </TableCell>
                                <TableCell className="text-slate-400 dark:text-slate-500 text-sm">{formatDate(product.createdAt)}</TableCell>
                                {isAdmin && (
                                    <TableCell>
